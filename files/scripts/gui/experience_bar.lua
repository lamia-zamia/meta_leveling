...
function EB:AnimateBarAlpha()
	...
	if self.data.anim.alpha > 0.75 or self.data.anim.alpha < 0 then self.data.anim.direction = self.data.anim.direction * -1 end
...
function EB:DrawExpBarOnLeft()
	...
	self:DrawExpFiller(self.bar.x + self.bar.scale_x, y + 1, self.bar.thickness * self.bar.scale_x, self.bar.scale_y - 1, true)
...
function EB:DrawExpBarOnRight()
	...
	self:DrawExpFiller(self.bar.x + self.bar.scale_x, self.bar.y + 1, self.bar.thickness * self.bar.scale_x, self.bar.scale_y - 1, true)
...
function EB:LevelUpFX()
	self.data.sound_played_level[ML:get_level()] = true
	if GameHasFlagRun(ML.const.flags.fx_played) then return end
	GameAddFlagRun(ML.const.flags.fx_played)
	if self.data.play_sound then GamePlaySound(ML.const.sound_banks.event_cues, "event_cues/wand/create", ML.player.x, ML.player.y) end
	if self.data.play_fx then EntityLoad("data/entities/particles/image_emitters/wand_effect.xml", ML.player.x, ML.player.y) end
end
